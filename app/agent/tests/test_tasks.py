...
def test_fetch_filtered_job_postings_tool(crew_instance):
    ...
    result_json = crew_instance.fetch_filtered_job_postings_tool(
        career_years=3, skills=["Python", "Django"]
    )
...
def test_analyze_resume_tool(crew_instance):
    ...
    resume = Resume.objects.create(user_id=1, content="Test resume content")
    ...
    result_json = crew_instance.analyze_resume_tool(
        user_id=1, analysis_result=analysis_data
    )
    result_data = json.loads(result_json)
...
def test_save_recommendations_tool(crew_instance):
    ...
    recommendations = [
        ...
        {"posting_id": 2, "rank": 2, "match_score": 90, "match_reason": "Okay fit"},
        {
            ...
            "rank": 3,
            "match_score": 88,
            "match_reason": "No data",
        },  # 존재하지 않는 공고
    ]

    result_json = crew_instance.save_recommendations_tool(
        user_id=1, recommendations=recommendations
    )
    result_data = json.loads(result_json)
...
def test_crew_kickoff_mocked(MockKickoff, crew_instance):
    ...
    mock_kickoff_result = {"status": "success", "final_output": "Some mocked result"}
