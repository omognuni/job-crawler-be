...
class SkillExtractionService:
    ...
    정규식 패턴 매칭을 사용하여 104개의 기술 스택을 식별합니다.
    """
    ...
    def extract_skills_from_job_posting(
        ...
        skills_required = SkillExtractionService.extract_skills(required_text)
...
# Backward compatibility: 기존 함수 형태 유지
def extract_skills(text: str) -> List[str]:
    """
    [Backward Compatibility] 기존 함수 호환용

    SkillExtractionService.extract_skills()를 호출합니다.
    """
...
def extract_skills_from_job_posting(
    ...
    SkillExtractionService.extract_skills_from_job_posting()를 호출합니다.
    """
    return SkillExtractionService.extract_skills_from_job_posting(
...
def get_all_skills() -> List[str]:
    ...
    SkillExtractionService.get_all_skills()를 호출합니다.
    """
    return SkillExtractionService.get_all_skills()
...
def get_skill_count() -> int:
    ...
    SkillExtractionService.get_skill_count()를 호출합니다.
    """
    return SkillExtractionService.get_skill_count()
